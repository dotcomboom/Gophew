    ...
    for line in robots.replace('\r\n', '\n').split('\n'):
        if line.startswith('Disallow:'):
            ...
            if line in req.path:
                allowed = False
    if allowed and (
        f'Disallow: {req.path}' not in robots
        and 'Disallow: *' in robots
        or f'Disallow: {req.path}' in robots
    ):
        allowed = False
    return allowed
...
def crawl(url, cooldown=86400):
    tocrawl = []
    req = pituophis.parse_url(url)
    if req.url() in db['menus'] and not (
        time.time() >= (db['menus'][req.url()]['last_crawled'] + cooldown)
    ):
        print('Not crawling', url, 'due to', f'{str(cooldown)}ms cooldown')
        return False
    try:
        if settings['limit_host'] and settings['limit_host'] != req.host:
            return False
        if not req.path.startswith(settings['path_must_start_with']):
            return False
        save()
        if req.type in crawl_types and allowed_to_crawl(req.url()):
            print('Waiting to crawl', f'{req.url()}...')
            ...
            for item in resp.menu():
                if item.type not in settings['ignore_types']:
                    ...
                    if (
                        settings['limit_host']
                        and settings['only_record_host']
                        and item.request().host != settings['limit_host']
                    ):
                        record = False
                    if not req.path.startswith(settings['path_must_start_with']):
                        record = False
                    ...
                    if record:
                        ...
                        if surl not in db['items']:
                            db['items'][surl] = {'titles': [], 'referrers': []}
